...
def _scrub_sensitive_body(body: str, content_type: str) -> str:
    ...
    if not body or content_type not in ['application/json', 'application/x-www-form-urlencoded']:
        return body
    ...
    for pattern in sensitive_patterns:
        scrubbed_body = re.sub(pattern, r'\1: "[REDACTED]"', scrubbed_body, flags=re.IGNORECASE)
...
def before_send(event: Dict[str, Any], hint: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if 'request' in event and 'headers' in event['request']:
            event['request']['headers'] = _scrub_sensitive_headers(event['request']['headers'])
        
        # Scrub request body
        if 'request' in event and 'data' in event['request']:
            content_type = event['request'].get('headers', {}).get('content-type', '')
            if isinstance(event['request']['data'], str):
                event['request']['data'] = _scrub_sensitive_body(event['request']['data'], content_type)
        
        # Scrub query string
        if 'request' in event and 'query_string' in event['request']:
            event['request']['query_string'] = _scrub_query_params(event['request']['query_string'])
        ...
        if not settings.should_send_default_pii:
            ...
            if 'contexts' in event and 'user' in event['contexts']:
                event['contexts']['user'] = {'id': event['contexts']['user'].get('id', '[REDACTED]')}
...
def init_sentry() -> None:
    ...
    try:
        ...
        logger.info(f"Sentry initialized successfully for environment: {settings.SENTRY_ENV}")
