...
class TenantUserScopeMiddleware(BaseHTTPMiddleware):
    ...
    def _extract_tenant_info(self, request: Request) -> Optional[dict]:
        ...
        try:
            ...
            tenant_header = request.headers.get('X-Tenant-ID') or request.headers.get('X-Organization-ID')
            if tenant_header:
                return {'id': tenant_header, 'source': 'header'}
            
            # Check for tenant in query parameters
            tenant_query = request.query_params.get('tenant') or request.query_params.get('organization')
            if tenant_query:
                return {'id': tenant_query, 'source': 'query'}
            
            # Check for tenant in path parameters
            tenant_path = request.path_params.get('tenant') or request.path_params.get('organization')
            if tenant_path:
