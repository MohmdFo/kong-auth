...
class KongAPITester:
    ...
    async def test_create_service(self):
        ...
        try:
            ...
            service = await self._make_request("POST", "/kong/services", json=service_data)
            if service.get("name") == "test-service":
                self.log_test("Create Service", True, f"Service created: {service['name']}")
                return True
            else:
            ...
            if e.response.status_code == 503:
                self.log_test("Create Service", False, "Kong Admin API not accessible - Kong may not be running")
                return False
            else:
                self.log_test("Create Service", False, f"HTTP {e.response.status_code}: {e.response.text}")
                return False
        except Exception as e:
    ...
    async def test_list_services(self):
        ...
        try:
            ...
            if isinstance(services, list):
                test_service = next((s for s in services if s.get("name") == "test-service"), None)
                if test_service:
                    self.log_test("List Services", True, f"Found {len(services)} services")
                    return True
                else:
                    self.log_test("List Services", False, "Test service not found in list")
                    return False
            else:
    ...
    async def test_get_service(self):
        ...
        try:
            ...
            if service.get("name") == "test-service":
                self.log_test("Get Service", True, f"Service retrieved: {service['name']}")
                return True
            else:
    ...
    async def test_list_routes(self):
        ...
        try:
            ...
            if isinstance(routes, list):
                test_route = next((r for r in routes if r.get("name") == "test-route"), None)
                if test_route:
    ...
    async def test_enable_plugin(self):
        ...
        try:
            ...
            plugin = await self._make_request("POST", "/kong/services/test-service/plugins", json=plugin_data)
            if plugin.get("name") == "cors":
                self.log_test("Enable Plugin", True, f"Plugin enabled: {plugin['name']}")
                return True
            else:
    ...
    async def test_service_health(self):
        ...
        try:
            health = await self._make_request("GET", "/kong/services/test-service/health")
            if "service" in health and "routes" in health and "plugins" in health:
                self.log_test("Service Health", True, f"Health status: {health.get('status', 'unknown')}")
                return True
            else:
    ...
    async def test_complete_service_setup(self):
        ...
        try:
            ...
            result = await self._make_request("POST", "/kong/services/complete", json=complete_data)
            if result.get("status") == "success":
                self.log_test("Complete Service Setup", True, "Complete service setup successful")
                return True
            else:
                self.log_test("Complete Service Setup", False, "Complete service setup failed")
                return False
        except Exception as e:
    ...
    try:
        async with KongAPITester(API_BASE_URL) as tester:
            ...
            if not success:
                ...
                print("=" * 60)
                print("If tests are failing due to connection errors, Kong may not be running.")
                print("")
                print("To start Kong for testing:")
                print("1. Run: ./start-kong-for-testing.sh")
                print("2. Or manually: docker-compose -f kong/docker-compose.kong.yml up -d")
